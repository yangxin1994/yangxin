...
class SohuUser < ThirdPartyUser
	...
	def self.get_access_token(code, redirect_uri)
		access_token_params = {"client_id" => OOPSDATA[RailsEnv.get_rails_env]["sohu_app_key"],
			"client_secret" => OOPSDATA[RailsEnv.get_rails_env]["sohu_app_secret"],
	...
	def self.save_tp_user(response_data)
		...
		if sohu_user.nil?
			...
			# update info 
			#sohu_user.update_by_hash(response_data)
		end
