...
class Suffrage
  ...
  def self.create_new(user_id,movie_id,vt)
  	suffrage =  self.where(vote_user_id:user_id,movie_id:movie_id).first
  	if suffrage.present?
  		suffrage.update_attributes(vote_type:vt)
  	else
  		suffrage = self.create(vote_user_id:user_id,movie_id:movie_id,vote_type:vt)	
  	end
