...
$(function() {
	...
	if (typeof(window.win_order_id) !== 'undefined' && window.win_order_id != '') {
		//中奖，已提交订单，刷新页面情况	
		$(".win").children('p').text("恭喜您！抽到了" + window.win_prize_title + "！").end().animate({
			"top": "65px"
		}, 800, "easeOutBounce");
	...
	$(".machine-start").mouseup(function() {
		if (!isplaying) {
			...
			for (var i = 0; i < 3; i++) {
				$(".icons-rotate" + i).animate({
					"top": "0px"
				}, parseInt(Math.random() * 800), "linear", rotate);
			};
		};
	...
	function get_recerver_info() {
		...
		if ($('div.address-slt').length < 1) {
			window.addressSelector = $.od.odAddressSelector({
				precision: 2, // 0. province, 1. city, 2. town, 3. detail 
				has_postcode: false,
				value: {
					address: window.sample_address,
					detail: '',
					postcode: ''
				}
	...
	function generate_orders(button, order_obj) {
		...
		$.postJSON('/orders/create_lottery_order.json', {
