$ ->
  ...
  $(".handle").click ->
    ...
    $.ajax
      url: "/admin/carnivals/#{order_id}/handle"
      type: 'PUT'
      success: (ret)->
        if ret.success
  ...
  $(".finishs").click ->
    ...
    order_id = $this.attr('href').split('-')[1]
    $.ajax
      url: "/admin/carnivals/#{order_id}/finish"
      type: 'PUT'
      data:
        success: true
      success: (ret)->
  ...
  $(".batch").click ->
    ...
    if confirm "确定要批量处理吗?"
      $.ajax
        url: "/admin/carnivals/orders/batch"
        type: 'PUT'
  ...
  $("#send_remark").click ->
    ...
    $.ajax
      url: "/admin/carnivals/#{orid}/update_remark"
      type: 'PUT'
      data: remark: _remark
      success: (ret)->
        if ret.success
  ...
  $("#send_info").click ->
    ...
    $.ajax
      url: "/admin/carnivals/#{$("#oid").val()}/update_express_info"
      method: 'PUT'
      data:
