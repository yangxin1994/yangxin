...
jQuery(function($) {
	...
	if(mdata.error_code){
		...
		total=0;
		alert("您没有管理员权限！");
		//location.reload("/old_signin");
	}

	//myPagination,返回当前页面显示的数据数
	var link = "/admin/messages";
	window.setPage('messages',total_page,link,curr_page,data_onePage,length);
	


	//show the data on the page
	//var length = total<data_onePage? total: data_onePage; 
	...
	//点击标题,显示内容		
	$('#tab1 tbody tr').find('td:eq(1)').children().click(function(e){
		e.stopPropagation();
	...
	//鼠标经过接收人显示列表
	$('#tab1 tbody tr').find('td:eq(4)').mouseover(function(e){
		e.stopPropagation();
