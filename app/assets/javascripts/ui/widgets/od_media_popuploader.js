...
(function($){
	...
	$.odWidget('odMediaPopuploader',{
		...
		fileData:[],
		chooseValue:{ids: [],links: []},//index存储在页面的id号，data存储此数据
		...
		uploadEvent:function(){
			...
			//链接 确定按钮
			this._find(".internetMedia .link_address").odEnter({enter:function(e){							
	...
	//从数据库中获得此用户上传的该类文件
		getUserFile:function(){
			this.fileData=[];
			var w=$.od.odWaiting({contentId:'.localMedia',addHeight:10});
		...
		chooseFile:function(){//显示并记录已选择的文件,在showFile中调用
			...
			this._find('.localMedia .tick-checkBtn').click($.proxy(function(e){
				...
				if(tick.hasClass('true')){ //点击后取消
					...
					this.chooseValue.ids[chooseId]={data:this.fileData[i],dataId:i};
	...
	//显示弹出框
		_show: function() {                        
			...
			this._find('.btn_close2').on("contextmenu",function(){return false;});            
			...
			for(var i=0; i<this.options.value.ids.length; i++){
				for(var j=0; j<this.fileData.length; j++){
					if(this.options.value.ids[i]==this.fileData[j]._id){
						this.chooseValue.ids[i]={data:this.fileData[j],dataId:j};
						break;
					}
				}
			}
