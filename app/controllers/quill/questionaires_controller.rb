...
class Quill::QuestionairesController < Quill::QuillController
  ...
  # GET
  def index
    @surveys = current.surveys.where(title: Regexp.new(params[:title].to_s))..where(:status.in => Tool.convert_int_to_base_arr(status))

    # add stars survey in Index action
    @stars = params[:stars].to_s == "true"
    @status = params[:status] || (Survey::CLOSED + Survey::PUBLISHED)
    if !params[:title].nil?
      @surveys = current_user.surveys.where(title: Regexp.new(params[:title].to_s)).desc(:created_at)
