...
class ApplicationController < ActionController::Base
  ...
  def require_sign_in
    if !user_signed_in
      respond_to do |format|
        format.html { redirect_to home_i_path({ref: request.url}) and return }
  ...
  def social_auth_link(website)
    ...
    when 'renren'
      return "https://graph.renren.com/oauth/authorize?client_id=#{client_id}&response_type=code&redirect_uri=#{redirect_uri}&display=page"
    when 'qq'
