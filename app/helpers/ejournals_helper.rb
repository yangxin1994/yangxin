...
module EjournalsHelper
  ...
  def ejournal_status(ejournal)
    ...
    when 0
      "草稿"
    when -1
      "发送中(#{ejournal['delivered_count']}/#{ejournal['all_sub_count']})"
    when 1
      "发送完成(#{ejournal['delivered_count']}/#{ejournal['all_sub_count']})"
    when -2
      "发送中断(#{ejournal['delivered_count']}/#{ejournal['all_sub_count']})"
    else
      "好歹有个状态啊?!"
    end
  end
