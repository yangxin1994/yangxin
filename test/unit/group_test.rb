...
class GroupTest < ActiveSupport::TestCase
	test "group creation" do
		...
		sub_groups = []
		retval = Group.check_and_create_new(jesse, "name_1", "description_1", [], [])
		sub_groups << retval["group_id"]
		retval = Group.check_and_create_new(jesse, "name_2", "description_2", [], [])
		sub_groups << retval["group_id"]
		retval = Group.check_and_create_new(jesse, "name_3", "description_3", [], [])
		sub_groups << retval["group_id"]
		retval = Group.check_and_create_new(jesse, "name_4", "description_4", [], [])
		sub_groups << retval["group_id"]

		members = generate_group_members

		retval = Group.check_and_create_new(oliver, "new group", "new group description", members, sub_groups)
		assert_equal ErrorEnum::UNAUTHORIZED, retval

		retval = Group.check_and_create_new(jesse, "new group", "new group description", Marshal.load(Marshal.dump(members)) << {}, sub_groups)
		assert_equal ErrorEnum::ILLEGAL_EMAIL, retval

		retval = Group.check_and_create_new(jesse, "new group", "new group description", members, Marshal.load(Marshal.dump(sub_groups)) << "wrong group id")
		assert_equal ErrorEnum::GROUP_NOT_EXIST, retval

		retval = Group.check_and_create_new(jesse, "new group", "new group description", members, sub_groups)
	...
	test "group update" do
		...
		retval = group_inst.update_group(oliver, updated_group_obj)
		assert_equal ErrorEnum::UNAUTHORIZED, retval
	
		retval = group_inst.update_group(jesse, updated_group_obj)
	...
	test "group delete" do
		...
		members = generate_group_members
		retval = Group.check_and_create_new(jesse, "new group", "new group description", [], [])
		group_id = retval["group_id"]
		parent_group = Group.check_and_create_new(jesse, "new group", "new group description", [], [retval["group_id"]])

		group_inst = Group.find_by_id(group_id)
		assert_equal Group, group_inst.class

		retval = group_inst.delete(oliver)
		assert_equal ErrorEnum::UNAUTHORIZED, retval

		retval = group_inst.delete(jesse)
		assert_equal retval, true
		retval = Group.find_by_id(group_id)
		assert_nil retval
		parent_group_inst = Group.find_by_id(parent_group["group_id"])
		parent_group_obj = parent_group_inst.show(jesse)
