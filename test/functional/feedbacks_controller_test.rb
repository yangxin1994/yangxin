...
class FeedbacksControllerTest < ActionController::TestCase
	...
	test "02 should post create action" do
		...
		post 'create', :feedback => {feedback_type: "Type1", title: "title1", content: "content1"}, :format => :json, :auth_key => auth_key
		assert_equal ErrorEnum::FEEDBACK_TYPE_ERROR.to_s, @response.body
		
		post 'create', :feedback => {feedback_type: 129, title: "title1", content: "content1"}, :format => :json, :auth_key => auth_key
		assert_equal ErrorEnum::FEEDBACK_RANGE_ERROR.to_s, @response.body

		post 'create', :feedback => {feedback_type: 1, title: "title1", content: "content1"}, :format => :json, :auth_key => auth_key
		...
		post 'create', :feedback => {content: "content1"}, :format => :json, :auth_key => auth_key
		assert_equal ErrorEnum::FEEDBACK_SAVE_FAILED.to_s, @response.body

		#
		# get index
		#
		post 'create', :feedback => {feedback_type: 2, title: "title2", content: "content2"}, :format => :json, :auth_key => auth_key
		post 'create', :feedback => {feedback_type: 64, title: "title3", content: "content3"}, :format => :json, :auth_key => auth_key
		post 'create', :feedback => {feedback_type: 128, title: "title4", content: "content4"}, :format => :json, :auth_key => auth_key
	...
	test "03 should post update action which is with admin" do
		...
		post 'update', :id => "123443454354353", :feedback => {title: "updated title1"}, :format => :json, :auth_key => auth_key
		assert_equal ErrorEnum::FEEDBACK_NOT_EXIST.to_s, @response.body

		post 'update',:id => feedback.id.to_s ,  :feedback => {feedback_type: "Type1", title: "title1", content: "content1"}, :format => :json, :auth_key => auth_key
		assert_equal ErrorEnum::FEEDBACK_TYPE_ERROR.to_s, @response.body
		
		post 'update',:id => feedback.id.to_s,  :feedback => {feedback_type: 129, title: "title1", content: "content1"}, :format => :json, :auth_key => auth_key
		assert_equal ErrorEnum::FEEDBACK_RANGE_ERROR.to_s, @response.body

		post 'update',:id => feedback.id.to_s,  :feedback => {feedback_type: 4, title: "title1", content: "content1"}, :format => :json, :auth_key => auth_key
		assert_equal ErrorEnum::FEEDBACK_NOT_CREATOR.to_s, @response.body

		sign_out(auth_key)

		auth_key = sign_in(user.email, "123456")
		post 'update', :id => feedback.id.to_s, :feedback => {title: "updated title1"}, :format => :json, :auth_key => auth_key
		retval = JSON.parse(@response.body)
