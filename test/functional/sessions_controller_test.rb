...
class SessionsControllerTest < ActionController::TestCase
	...
	test "should login" do
		...
		result = JSON.parse(@response.body)
		assert User.find_by_email(jesse.email).auth_key, result["value"]["auth_key"]
	...
	test "should create new password" do
		...
		get :new_password, :format => :json, :password => "new_password", :password_confirmation => "new_password", :password_key => "wrong password key"
		result = JSON.parse(@response.body)
		assert_equal false, result["success"]

		get :new_password, :format => :json, :password => "new_password", :password_confirmation => "new_password", :password_key => password_key_1
		result = JSON.parse(@response.body)
		assert_equal ErrorEnum::RESET_PASSWORD_EXPIRED.to_s, result["value"]["error_code"]

		get :new_password, :format => :json, :password => "new_password", :password_confirmation => "wrong confirmation", :password_key => password_key_2
		result = JSON.parse(@response.body)
		assert_equal ErrorEnum::WRONG_PASSWORD_CONFIRMATION.to_s, result["value"]["error_code"]
		
		get :new_password, :format => :json, :password => "new_password", :password_confirmation => "new_password", :password_key => password_key_2
		result = JSON.parse(@response.body)
		assert_equal true, result["value"]
	end
end
