...
class Admin::SystemUsersControllerTest < ActionController::TestCase
	...
	test "02 post create" do 
		...
		post 'create', :system_user => {username: "lisi", password: "123456", system_user_type: 2, true_name:'lisi'}, :format => :json, :auth_key => auth_key
		retval = JSON.parse(@response.body)
		...
		assert_equal ErrorEnum::SYSTEM_USER_MUST_EMAIL_OR_USERNAME.to_s, @response.body

		post 'create', :system_user => {email: "test4@example.com", system_user_type: 2, true_name:'lisi'}, :format => :json, :auth_key => auth_key
		assert_equal ErrorEnum::SYSTEM_USER_SAVE_FAILED.to_s, @response.body
	...
	test "05 post unlock" do
		...
		auth_key = sign_in(user.email, "123456")		
		post 'create', :system_user => {username: "zhangsan", password: "123456", true_name:'zhangsan', lock: true}, :format => :json, :auth_key => auth_key
		retval = JSON.parse(@response.body)
		assert_equal retval["username"], "zhangsan"
		assert_equal retval["lock"], true
